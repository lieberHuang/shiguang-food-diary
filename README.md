...
- 顶部汇总蛋白质、碳水、脂肪和热量，支持调整每日目标。
- 周日期切换、日期选择、早餐/午餐/晚餐/加餐归类。
- 拍照、选择照片、拖放图片；压缩本地缩略图。
- 明确标注的固定示例解析；手动录入；调整份量时同比换算。
- 编辑、删除和撤销删除；记录保存在当前浏览器 localStorage。
- 首次访问提供当天的三条示例记录，均可修改或删除。
- 仅提供移动端单列布局，固定底部拍照/手动记录入口；电脑打开时保持手机宽度，不提供桌面版布局。

## 接入真实后台

当前不包含 AI 模型、服务器、账户或云端数据库。示例模式不会上传照片，营养值不由照片推断。首次目标与食物数据均为演示值。
...
页面在支持 `document.modelContext` 的环境中提供 `get_daily_nutrition` 和 `start_food_record`；无兼容验证上下文，未验证 WebMCP 运行时。普通浏览器不受影响。
